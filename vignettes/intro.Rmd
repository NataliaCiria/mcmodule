...
The risk assessment for our cattle purchase can be performed using base R [@base] random sampling functions, or `mc2d` [@mc2d], a package provides additional probability distributions (such as rpert) and other useful tools for analyzing stochastic (Monte-Carlo) simulations.
...
In these visualizations, input datasets appear in <h color="blue">blue</h>, input columns in <h color="gray">dark-grey-blue</h>, input mcnodes in <h color="gray">grey</h>, output mcnodes in <h color="green">green</h>, and total mcnodes (as we will see later) in <h color="orange">orange</h>. The numbers displayed when clicked correspond to the first variate of each mcnode.
...
```{r}
    ...
    combined_module<-combine_modules(imports,current_module)
