...
For an application of this risk assessment approach to on-farm biosecurity, see the [model description](https://github.com/NataliaCiria/biosecurity_model/blob/main/DOCUMENTATION/model_description.pdf) file. Details of the functions that are part of the `mcmodule` package can be found in the package documentation.

# Multivariate Monte-Carlo simulations

*The following section is based primarily on the mc2d package [vignette](https://cran.r-project.org/web/packages/mc2d/vignettes/docmcEnglish.pdf), adapted and expanded specifically for the context of modular, flexible Monte Carlo simulations.*
...
![Structure of an mcnode containing u uncertainty iterations and i variates.](images/mcmodule_mcnode_definition.png){alt="Structure of an mcnode containing u uncertainty iterations and i variates." fig-align="center" width="40%"}
...
![An mcnode representing herd prevalence of three pathogens, tuberculosis (TB), infectious bovine rhinotracheitis (IBR), and bovine viral diarrhea (BVD), and two animal categories (cows and heifers). The herd prevalence is the same for both animal categories, but values differ due to the stochastic nature of random sampling from the PERT distribution.](images/mcmodule_mcnode_example.png){alt="An mcnode representing herd prevalence of three pathogens, tuberculosis (TB), infectious bovine rhinotracheitis (IBR), and bovine viral diarrhea (BVD), and two animal categories (cows and heifers). The herd prevalence is the same for both animal categories, but values differ due to the stochastic nature of random sampling from the PERT distribution." fig-align="center" width="85%"}
...
The scenario id, group id, and row number are the three mcnode properties that enable row matching and row aggregation operations (see @sec-row-matching-operations).

![An mcnode representing the probability of a farm being infected in a baseline scenario ("0", in blue) and in a what-if scenario where there is a control plan implemented ("a", in green).](images/mcmodule_mcnode_scenarios.png){alt="An mcnode representing the probability of a farm being infected in a baseline scenario (\"0\", in blue) and in a what-if scenario where there is a control plan implemented (\"a\", in green)." fig-align="center" width="70%"}
...
![](images/mcmodule_mcnode_element-wise_operation.png){fig-align="center" width="100%"}

### Row matching {#sec-row-matching-operations}
...
![](images/mcmodule_group_match_1.png){fig-align="center" width="100%"}

2.  Assign common group ids, based on keys

![](images/mcmodule_group_match_2.png){fig-align="center" width="100%"}

3.  Reorder rows to align group ids. This is similar to dplyr `left_join`

![](images/mcmodule_group_match_3.png){fig-align="center" width="100%"}
...
![](images/mcmodule_scenario_match_1.png){fig-align="center" width="100%"}

2.  For missing scenarios, duplicate the baseline scenario (scenario "0")

![](images/mcmodule_scenario_match_2.png){fig-align="center" width="100%"}
...
![](images/mcmodule_scenario_match_3.png){fig-align="center" width="100%"}
...
![](images/mcmodule_null_matching_1.png){fig-align="center" width="100%"}

2.  Add null variates with probability 0 for any missing groups (and apply scenario matching)

![](images/mcmodule_null_matching_2.png){fig-align="center" width="100%"}
...
![](images/mcmodule_null_matching_3.png){fig-align="center" width="100%"}

### Row aggregation {#row-aggregation-operations}
...
![Row aggregation by pathogen. The calculation is performed element-wise across subset variates to maintain uncertainty propagation.](images/mcmodule_row_aggregation_p.png){alt="Row aggregation by pathogen. The calculation is performed element-wise across subset variates to maintain uncertainty propagation." fig-align="center" width="100%"}
...
$$ set\_p= 1-(1-subset\_p(1-(1-trial\_p)^{trial\_n}))^{subset\_n}  $$

**Multiple group multilevel trials**

### At least one

A special type of element-wise operation that automatically deals with row matching.
