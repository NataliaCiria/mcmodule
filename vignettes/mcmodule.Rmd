...
5.  Perform model diagnosis such as convergence and sensitivity analysis
...
imports_wif$node_list$no_detect_set_rrr$summary
```
...
The `mcmodule_corr()` function calculates correlation coefficients between input and output nodes across your mcmodule, and `mcmodule_tornado()` creates a plot to visualize this correlations. This helps identify which input parameters have the strongest influence on your results. It is based on `mc2d::tornado()` but can be applied to multivariate inputs.
...
Sensitivity analysis quantifies how changes in inputs affect model outputs, helping to identify which uncertainties are most important for decision-relevant results. In `mcmodule`, you can perform sensitivity analysis using sample design-based methods such as Morris or Sobol indices for global sensitivity analysis. Different methods using `sensitivity` [@sensitivity] or `sensobol` [@sensobol] R packages are described in the [Sensitivity analysis vignette](https://nataliaciria.com/mcmodule/articles/sensitivity_analysis.html).
...
A similar function is `mcnode_null_rm()`, that allows you replace missing nodes, required a in expression but not found in the input data with an value (usually zero).

## Customizing total node names

Functions like `at_least_one()`, `agg_variates()`, and `trial_totals()` automatically generate new mcnodes with informative suffixes. You can customize these names for clarity or documentation purposes:

```{r}
# Custom name for at_least_one()
intro <- at_least_one(intro, c("no_detect", "dir_contact"), name = "custom_total")

# Custom name for agg_variates()
intro <- agg_variates(intro, "no_detect_set", name = "custom_agg")

# Custom suffix for agg_variates()
intro <- agg_variates(intro, "no_detect_set",
...
You can transform a full mcmodule into a list of matrices or of `mc2d` `mc` objects with `mcmodule_to_matrices()` and `mcmodule_to_mc()` respectively.
